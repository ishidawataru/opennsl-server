...
void do_some_more_tests()
{
	...
	{
		throw s11n::s11n_exception( "%s:%d: ser_nullary_f test failed :(", __FILE__,__LINE__);
	}
	CERR << "ser_nullary_f():\n";
	s11nlite::save( node, std::cout );
	...
	{
		throw s11n::s11n_exception( "%s:%d: deser_nullary_ test failed :(", __FILE__,__LINE__);
	}
	...
	{
		throw s11n::s11n_exception( "%s:%d: deser_nullary_f test failed :(", __FILE__,__LINE__);
	}
	CERR << "deser_nullary_f():\n";
	s11nlite::save( unmap, std::cout );
	...
	{
		throw s11n::s11n_exception( "%s:%d: logical_and() test failed :(", __FILE__,__LINE__ );
	}
